...
public class RoadOnScene : BuildingOnScene
{
   ...
   public Vector2Int[] _cellCenters;
    ...
    void AddRoadQuadForCell(Vector2Int cellCenter, List<Vector3> vertices, List<int> triangles, List<Vector2> uvs)
    {
        float yPos = 0.05f;
        ...
        Vector3 bottomLeft = new Vector3(worldCenter.x - halfRoadWidth, yPos, worldCenter.z - halfRoadWidth);
        Vector3 bottomRight = new Vector3(worldCenter.x + halfRoadWidth, yPos, worldCenter.z - halfRoadWidth);
        Vector3 topLeft = new Vector3(worldCenter.x - halfRoadWidth, yPos, worldCenter.z + halfRoadWidth);
        Vector3 topRight = new Vector3(worldCenter.x + halfRoadWidth, yPos, worldCenter.z + halfRoadWidth);
    ...
    void AddRoadSegment(Vector2Int startCell, Vector2Int endCell, List<Vector3> vertices, List<int> triangles, List<Vector2> uvs, int segmentIndex)
    {
        float yPos = 0.05f;
    ...
    public void DrawDebugBounds(Vector2Int[] cellCenters)
    {
        ...
        foreach (var cell in cellCenters)
        {
            ...
            Debug.DrawLine(worldCenter - Vector3.forward * 0.3f, worldCenter + Vector3.forward * 0.3f, Color.red, 2f);
            Debug.DrawLine(worldCenter - Vector3.right * 0.3f, worldCenter + Vector3.right * 0.3f, Color.red, 2f);
        }
    }
}