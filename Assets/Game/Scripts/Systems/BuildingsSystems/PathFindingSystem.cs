...
public partial class PathfindingSystem  : SystemBase
{
    ...
    Dictionary<(int2 start, int2 end), NativeList<int2>> _pathCache = new Dictionary<(int2, int2), NativeList<int2>>();
    ...
    public void FindBuildingPathAsync(int2 start, int2 end, Action<NativeList<int2>> onComplete)
    {
        ...
        bool startBlocked = buildingMap.CellMapBuildings.TryGetValue(start, out int startID) && startID != _roadHash;
        bool endBlocked = buildingMap.CellMapBuildings.TryGetValue(end, out int endID) && endID != _roadHash;
   ...
   void InternalFindPathAStar(int2 start, int2 end, Action<NativeList<int2>> onComplete)
    ...
    {
        [ReadOnly] public NativeParallelHashMap<int2, int> obstacles;
        [ReadOnly] public int roadHash;
        public int2 start;
        public int2 goal;
        public NativeList<int2> result;
        ...
        public void Execute()
        {
            ...
            while (openSet.Length > 0)
            {
                ...
                for (int i = 0; i < directions.Length; i++)
                {
                    ...
                    bool blocked = obstacles.ContainsKey(neighbor) && obstacles[neighbor] != roadHash;
                    if (blocked) continue;
                    ...
                    float tentativeG = gScore.ContainsKey(current) ? gScore[current] + stepCost : float.MaxValue;
                    ...
                    if (!gScore.TryGetValue(neighbor, out float neighborG) || tentativeG < neighborG)
                    {
                        ...
                        for (int j = 0; j < openSet.Length; j++)
                            if (openSet[j].Equals(neighbor)) { inOpenSet = true; break; }
    ...
    bool IsGoalNeighbor(int2 current)
        {
            ...
            return current.Equals(right) || current.Equals(left) || current.Equals(up) || current.Equals(down);
        }
    ...
    protected override void OnDestroy()
    {
        ...
        foreach (var pj in _pendingJobs)
            combined = combined.Equals(default) ? pj.handle : JobHandle.CombineDependencies(combined, pj.handle);
        if (!combined.Equals(default)) combined.Complete();

        foreach (var pj in _pendingJobs)
            if (pj.result.IsCreated) pj.result.Dispose();
        _pendingJobs.Clear();
