...
{
    [Inject]
    BuildingObjectFactorty _buildingFactory;

    [Inject]
    PhantomObjectFactory _phantomFactory;
    ...
    protected override void OnUpdate()
    {
        if (!SystemAPI.HasSingleton<BuildingMap>())
            return;
        if (_buildingFactory == null || _phantomFactory == null)
            return;
        ...
        using (
            var mapCopy = new NativeParallelHashMap<int2, Entity>(
                buildingMap.ValueRO.CellEntity.Count(),
                Allocator.TempJob
            )
        )
        {
    ...
    {
        foreach (
            var (gameObjectRef, mewpoints, entity) in SystemAPI
                .Query<GameObjectReference, DynamicBuffer<MapPointData>>()
    ...
    {
        foreach (
            var (buildingPoints, buildingData, posData, entity) in SystemAPI
                .Query<DynamicBuffer<MapPointData>, BuildingData, BuildingPosData>()
                .WithAll<CreateVisualTag, CreateBuildingEventComponent>()
        ...
        foreach (
            var (buildingPoints, buildingData, entity) in SystemAPI
                .Query<DynamicBuffer<MapPointData>, BuildingData>()
    ...
    {
        foreach (
            var (gameObjectRef, entity) in SystemAPI
                .Query<GameObjectReference>()
    ...
    void CreateBuildingVisual(
        Entity entity,
        BuildingData buildingData,
        BuildingPosData posData,
        DynamicBuffer<MapPointData> buildingPoints,
    ...
    )
    {
        ...
        var buildingVisual = _buildingFactory.CreateBuilding(
            buildingData.BuildingIDHash,
            pos,
            posData.Rotation
        );
        ...
        ecb.AddComponent(building, new GameObjectReference { gameObject = buildingOnScene });
        ...
        if (EntityManager.HasComponent<BluePrint>(entity))
            ecb.AddComponent(building, new MakePhantomTag());
    ...
    void CreateRoadVisual(
        Entity entity,
        BuildingData buildingData,
        DynamicBuffer<MapPointData> roadPoints,
        EntityCommandBuffer ecb,
        NativeParallelHashMap<int2, Entity> map
    )
    {
